...
- 审计图表复制降级处理：浏览器因 HTTP 访问或权限限制拒绝复制图片时，系统会自动下载 PNG 文件，按钮提示“已下载PNG”，避免用户只看到复制失败却没有可用图片。

## 2026-05-18 更新（v2.28）

- 审计问题分析图表复制修复：复制图片时增加截图、生成 PNG、写入剪贴板三段超时保护，浏览器剪贴板卡住时会自动回到“复制失败”，不再一直显示“复制中”。
- AI 任务完成提醒扩展：培训统计、案件台账、授权请示、三台账合并、审计问题分析等长时间处理流程完成或失败时，均会触发页面提醒、浏览器标题闪动和可选系统通知。

## 2026-05-15 更新（v2.27）

- AI 任务完成提醒：合规审查台账提取、写入完成或失败时，页面右上角弹出提示，并让浏览器标题短暂闪动，用户切到其他窗口时也更容易注意到处理结果。
- 浏览器系统通知：提醒弹窗中提供“开启系统通知”，用户授权后，后续任务完成可弹出 Windows/浏览器通知；未授权或浏览器不支持时，仍保留页面提醒和标题闪动。
